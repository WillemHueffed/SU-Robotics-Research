...
bool interbotix_xs_msgs__msg__joint_temps__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("interbotix_xs_msgs.msg._joint_temps.JointTemps", full_classname_dest, 46) == 0);
  }
  interbotix_xs_msgs__msg__JointTemps * ros_message = _ros_message;
  {  // names
  ...
  {  // temps
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int32_t * dest = ros_message->temps.data;
...
{
  /* NOTE(esteve): Call constructor of JointTemps */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("interbotix_xs_msgs.msg._joint_temps");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "JointTemps");
  ...
  {  // temps
    ...
    if (ros_message->temps.size > 0) {
      ...
      int32_t * src = &(ros_message->temps.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->temps.size * sizeof(int32_t));
      assert(data != NULL);
